...
async function onPaymentFailed(intent) {
  ...
  await recordBooking({
    ...
    weeks: intent.metadata?.weeks || '',
    areaOfFocus: intent.metadata?.area_of_focus || '',
    // NOTE: this event only carries the PaymentIntent, not the Checkout Session,
    // so we match on paymentIntentId (set on the pending row at checkout creation)
    // rather than sessionId — otherwise this would create a duplicate booking
    // instead of updating the pending one.
