...
export async function getBookings(req, res) {
  try {
    ...
    const abandoned = failed.filter((r) => r.failureType === 'abandoned');
    const declined = failed.filter((r) => r.failureType === 'declined');
    const totalRevenue = successful.reduce((sum, r) => sum + (r.amount || 0), 0);
...
export async function getInquiries(req, res) {
  try {
    ...
    return res.status(200).json({ total: inquiries.length, inquiries });
  } catch (err) {
    console.error('MongoDB error (getInquiries):', err.message);
