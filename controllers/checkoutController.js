...
export async function createCheckout(req, res) {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${process.env.FRONTEND_URL}/${cancelPath}`,
      // Expand so we get the PaymentIntent id now — payment_intent.payment_failed
      // webhook events only carry the intent, not the Checkout Session, so we need
      // this id up front to match a later 'declined' event back to this same booking.
      expand: ['payment_intent'],
    });

    console.log(`✅ Checkout created — ${metadataProduct}${discountNote}`);

    // Write the pending booking now, not just on webhook resolution — otherwise
    // in-progress and abandoned checkouts never show up in the admin panel at all
    // (an abandoned Checkout Session only fires `checkout.session.expired` ~24h later).
    await recordBooking({
      status: 'pending',
      product: metadataProduct,
      ...
      weeks: isAccelerator ? String(parseInt(weeks, 10)) : '',
      areaOfFocus: isDiscoveryIntensive ? (areaOfFocus || '').trim() : '',
